...
function ReviewColumn({reviews, className, reviewClassName, msPerPixel = 0} : {
    ...
    style={{'--marquee-duration':duration} as React.CSSProperties}>{reviews.concat(reviews).map((imgSrc, reviewIndex) =>(
...
function RevieGrid() {
    ...
    return <div ref={containerRef} className="relative -mx-4 mt-16 grid h-[49rem] max-h[150vh] grid-cols-1 items-center gap-8 overflow-hidden px-4 sm:mt-20 md:grid-cols-2 lg:grid-cols-3">
        {isInView ? <>
            ...
            <ReviewColumn reviews={[...column1, ...column3[1]]} className="hidden md:block" reviewClassName={(reviewIndex)=> reviewIndex >= column2.length ? "lg:hidden" : ""} msPerPixel={15}/>
...
export function Reviews() {
    return(
        <MaxWidthWrapper className="relative max-w-5xl">
            <img src="/what-people-are-buying.png" alt="what people are buying" className="absolute select-none hidden xl:block -left-32 top-1/3" aria-hidden="true" />
            <RevieGrid/>
        </MaxWidthWrapper>
    )
}